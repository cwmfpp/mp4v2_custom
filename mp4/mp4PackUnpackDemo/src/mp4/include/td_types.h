...
typedef struct
{
	int m_iOsdType;					//Osd叠加类型
	int m_iOsdFontSize;				//字号大小1 时间日期 2 通道名称(文本) 3 logo颜色										// 4 附加字符 5 交通专用
}TChnOsdSizeCnf;
...
typedef struct		// 数字通道信息
{
	...
	td_s32 m_iServerType;			//设备厂家	GUI_DEV_TYPE_PRIVATE,GUI_DEV_TYPE_ONVIF,GUI_DEV_TYPE_PUSH,GUI_DEV_TYPE_RTSP
	td_u8 m_strServerName[40];		//服务器名称(暂时保留)
	td_u8 m_strServerIP[40];			//服务器地址，可以是IP地址，也可以是域名；  m_strUrl
	td_u8 m_strServerProxy[40];		//服务器上级代理m_strProxy
...
typedef struct
{
	int m_iGKGroupId;							//gk编号
	int m_iGkType;								//gk模式
	int m_iEncType;								//gk加密方式
	int m_iGkPort;								//gk端口
	char m_cGkRegName[STR_SERVER_NAME_LEN + 1];	//gk注册名称
	char m_cGkIP[STR_SERVER_NAME_LEN + 1];		//gk地址
}TH323GKPara;//323网守参数

typedef struct
{
	int m_iLocalGroupId;						//本机编号
	int m_iListenPort;							//本机监听端口
	int m_iIdCodeType;							//ID编码方式
	int m_iAnwTypeForCall;						//点对点呼叫响应方式
	int m_iMainSrcChnNo;						//编码主流通道号
	int m_iSubSrcChnNo;							//编码副流通道号
	int m_iMainDecChnNo;						//解码主流通道号
	int m_iSubDecChnNo;							//解码副流通道号
	char m_cLocalNo[STR_SERVER_NAME_LEN + 1];	//终端号码
	char m_cPassWord[STR_SERVER_NAME_LEN + 1];	//加密密码
}TH323LocalPara;//323本地参数
//add end
...
#define MAX_WORDOVERLAY_NUM	5	//政法暂时只支持一块，modify by liuqi 2014-7-18
// for decoder
typedef struct
{
...
#define M3G_PHONE_NUM      5//电话号码个数
#define M3G_PHONE_LEN      31//电话号码长度
#define M3G_SMS_LEN        160//短信长度
#define M3G_VPDN_DIAL_LEN  31//VPDN 拨号帐号长度
#define M3G_TIME_NUM       3//时间
...
typedef struct st3ginfo						//add zhangyi 2010/10/18
{
	//3G_DIALTYPE,//拨号方式
	T3G_DIAL_TYPE m_tStartType;	//拨号方式      
	T3G_OFF_TYPE  m_tStopType;	//断线方式
	int  m_iDuration;			//自动断线时常
	
	//3G_PHONE,//电话拨号参数
	char m_cPhone[M3G_PHONE_NUM][M3G_PHONE_LEN+1];			//电话白名单
	
	//3G_NOTIFY,//拨号成功通知
	int  m_iSmsNotifyType;   		//设置3G短信通知类型.	0，不通知；1，拨号成功后通知；2，拨号失败后通知
	char m_cNotifyMsg[M3G_SMS_LEN+1];     //和电话拨号共用
	
	//3G_VPDN,//设置VPDN帐号
	char m_cApn[M3G_VPDN_DIAL_LEN+1];       //VPND    add wanghailong 20101019
	char m_cAccount[M3G_VPDN_DIAL_LEN+1];		
	char m_cPassWord[M3G_VPDN_DIAL_LEN+1];
	
	//3G_TIME_DIAL,//设置定时拨号参数
	int m_iStartTime[M3G_TIME_NUM];	//0:Hour 1:Minute 2:Second
	int m_iStopTime[M3G_TIME_NUM];
	int m_iDialEnable;     //拨号使能
}ST3G_INFO;

typedef enum											//add zhangyi 2010/10/18												
{
	T3G_NOTONLINE = 0,//不在线
	T3G_ON_LINE,//在线
	T3G_DIAL,//拨号中
...
#define M3G_IP_LEN         31//ip长度
typedef struct st3gstat
{
	T3G_DEV_TYPE m_tDevType;									// 设备类型
	T3G_ONLINE_TYPE m_iOnline;										 	// 在线状态
	int m_iIntensity;											// 信号强度0-100
	char m_cPppIp[M3G_IP_LEN+1];									//拨号IP
	time_t m_iTime;											//初始拨号时间
	int m_iNotifyType;                                          //通知类型
}ST3G_STAT;
